...
def main():
    ...
    for n in 2**np.mgrid[2:9]:
        ...
        tloop = min(timeit.repeat(lambda: mm_loop_based(A, B, C), repeat=nruns, number=1))
        tblas1 = min(timeit.repeat(lambda: mm_blas1(A, B, C), repeat=nruns, number=1))
        tblas2 = min(timeit.repeat(lambda: mm_blas2(A, B, C), repeat=nruns, number=1))
        tblas3 = min(timeit.repeat(lambda: mm_blas3(A, B, C), repeat=nruns, number=1))
